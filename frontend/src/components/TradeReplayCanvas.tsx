...
export const TradeReplayCanvas: React.FC<TradeReplayCanvasProps> = ({ tradeId = "TRD-DEFAULT" }) => {
  ...
  useEffect(() => {
    setIsLoading(true);
    fetch(`${apiBase()}/api/v1/replay/session/${tradeId}`)
      .then((res) => res.json())
  ...
  const handleStep = async (dir: number) => {
    if (!session) return;
    try {
      const res = await fetch(`${apiBase()}/api/v1/replay/${session.session_id}/step`, {
        method: "POST",
  ...
  const handleSeek = async (idx: number) => {
    if (!session) return;
    try {
      const res = await fetch(`${apiBase()}/api/v1/replay/${session.session_id}/seek`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
