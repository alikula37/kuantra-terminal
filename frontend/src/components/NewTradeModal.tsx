...
export const NewTradeModal: React.FC<NewTradeModalProps> = ({ isOpen, onClose }) => {
  ...
  const [entryPrice, setEntryPrice] = useState<number>(currentPrice || 65000);
  const [qty, setQty] = useState<number>(1.0);
  const [stopLoss, setStopLoss] = useState<number>(currentPrice ? currentPrice * 0.98 : 63700);
  const [takeProfit, setTakeProfit] = useState<number>(currentPrice ? currentPrice * 1.04 : 67600);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
      <div className="bg-[#111722] border border-surface-border rounded-lg w-full max-w-md p-5 shadow-2xl">
        <div className="flex items-center justify-between border-b border-surface-border pb-3">
          <h3 className="text-sm font-bold text-white font-mono">RECORD TRADE TICKET</h3>
        ...
        <form onSubmit={handleSubmit} className="mt-4 space-y-4 text-xs font-mono">
          ...
          <button
            type="submit"
            className="w-full py-2 bg-accent hover:bg-sky-400 text-black font-bold rounded flex items-center justify-center space-x-2 transition"
          >
