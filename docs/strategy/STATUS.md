...
user toggle. Focused Journal/Theme DOM tests and tri-locale parity are passing. A new
pilot DMG/Release asset is still required before these changes are distributed to pilot
users; no user data was changed by the tests.
...
| P1-WP29 | IN PROGRESS | Architecture-scoped trusted pilot package, exact evidence bundle, manifest/checksums, user instructions and bounded pilot UI corrections | Clean arm64 package from `119ae57` passed locked local CI, exact `hdiutil verify`, read-only mounted-DMG WKWebView smoke, native manual-update click-through and manifest/checksum validation. The current source additionally contains the theme compatibility/race guard and confirmed audit-safe journal cancellation action; a new arm64 DMG/Release asset must be built and refreshed before pilot users receive these UI fixes. Default dual package remains blocked until native x86_64 evidence; N03 clean second profile/host, N05 signing/notarization and pilot-user read access remain open. |
