...
| H07 bounded baseline | Code `3de57c5` on clean Mac arm64: 709 backend, 87 frontend, i18n 596/596, local CI `MERGE READY`; legacy and exact-coverage typed trade queries abort mid-stream at the cooperative resource boundary without returning partial results. Evidence Pack/Reconciliation Inbox/Weekly Review/CSV preview plus Dashboard/Quant Analytics/Header portfolio and JournalView trade-list read surfaces have bounded AbortSignal, stale-response suppression and explicit loading/error truth; malformed successful payloads fail closed and unavailable analytics never becomes a zero scorecard or empty journal. Exact DMG/WKWebView smoke is PASS with provenance COMPLETE: report `94fe05ae...`, DMG `58dd9f0d...`, mounted executable `0f444063...`. Previous artifact-bound 100k Evidence Pack p95 `4530.5968 ms`, export p95 `4545.3095 ms`, projection rebuild p95 `8347.3076 ms`, max operation RSS `336.3438 MB` remains a non-release baseline; the `<2s` planning target is not met and MAE/MFE/other core read cancellation truth remains open |
...
| H07 | IMPLEMENTATION_REQUIRED | Deterministic 1k/10k/100k synthetic performance baselines, resource limits and explicit cancellation/failure truth | `f0b93ba` benchmark/index path, `779e2d7` atomic grouped-batch cancellation, `d2463b2` streamed integrity verification, `4e85e3f` correction/replay measurement, `46531e4`/`6af4fd9` dynamic budget boundary, `6748d96` bounded input/coverage fail-closed fixtures, `5137383` legacy/typed query mid-operation abort, `27b3404` bounded value-chain frontend cancellation, `42d67c6` dashboard/analytics/header read truth and `3de57c5` JournalView trade-list read truth are present. 100k Evidence Pack/rebuild p95 exceeds planning target; MAE/MFE and other core read cancellation/loading/error truth remain open |
...
fixtures, `42d67c6` dashboard/analytics/header read truth fixtures and `3de57c5`
JournalView trade-list read truth fixtures are present. Exact Mac arm64
app/DMG/WKWebView evidence is tied to code baseline `3de57c5`; DMG smoke report SHA
`94fe05ae1e67c55201d9ccea317275adebb4379471c11f107b71cecdc531e3f7`, mounted
executable SHA `0f444063f90bad198ba5ff5096e1c0ec9f2eab9e0265ba3e72011c51fe64abea`
and DMG SHA `58dd9f0dca4537edcf412d0edd19e5a9f1df3383b909d9fabce314255662b2d5`.
The 100k Evidence Pack and projection rebuild p95 values do not meet the planning
target, so H07 remains `IMPLEMENTATION_REQUIRED`. Next handoff is MAE-MFE and
other core long-read loading/cancellation/error truth audit.
