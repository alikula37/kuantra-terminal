...
| H07 bounded baseline | Code `42d67c6` on clean Mac arm64: 709 backend, 84 frontend, i18n 591/591, local CI `MERGE READY`; legacy and exact-coverage typed trade queries abort mid-stream at the cooperative resource boundary without returning partial results. Evidence Pack/Reconciliation Inbox/Weekly Review/CSV preview plus Dashboard/Quant Analytics/Header portfolio read surfaces have bounded AbortSignal, stale-response suppression and explicit loading/error truth; malformed successful payloads fail closed and unavailable analytics never becomes a zero scorecard. Exact DMG/WKWebView smoke is PASS with provenance COMPLETE: report `4cc9fe7d...`, DMG `f992806b...`, mounted executable `76d92760...`. Previous artifact-bound 100k Evidence Pack p95 `4530.5968 ms`, export p95 `4545.3095 ms`, projection rebuild p95 `8347.3076 ms`, max operation RSS `336.3438 MB` remains a non-release baseline; the `<2s` planning target is not met and JournalView/MAE-MFE/other core read cancellation truth remains open |
...
| H07 | IMPLEMENTATION_REQUIRED | Deterministic 1k/10k/100k synthetic performance baselines, resource limits and explicit cancellation/failure truth | `f0b93ba` benchmark/index path, `779e2d7` atomic grouped-batch cancellation, `d2463b2` streamed integrity verification, `4e85e3f` correction/replay measurement, `46531e4`/`6af4fd9` dynamic budget boundary, `6748d96` bounded input/coverage fail-closed fixtures, `5137383` legacy/typed query mid-operation abort, `27b3404` bounded value-chain frontend cancellation and `42d67c6` dashboard/analytics/header read truth are present. 100k Evidence Pack/rebuild p95 exceeds planning target; JournalView, MAE-MFE and other core read cancellation/loading/error truth remains open |
...
fixtures and `42d67c6` dashboard/analytics/header read truth fixtures are present.
Exact Mac arm64 app/DMG/WKWebView evidence is tied to code baseline `42d67c6`;
DMG smoke report SHA
`4cc9fe7d1a1d834837071d7fd6091462ad50ca7314aa783748a6d7ad329e926f`, mounted
executable SHA `76d927600013f0e33b5bc70e1a8a7bd768e467277e80bccaf720a68f50c177ef`
and DMG SHA `f992806baf458bdc74b32c78a0a569dff0c077ac8c924ce2a690e59ab0314637`.
The 100k Evidence Pack and projection rebuild p95 values do not meet the planning
target, so H07 remains `IMPLEMENTATION_REQUIRED`. Next handoff is JournalView,
MAE-MFE and other core long-read loading/cancellation/error truth audit.
