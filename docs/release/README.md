...
[`truth-matrix.v1.1.6.json`](truth-matrix.v1.1.6.json). The matrix is a contract, not a
...
The current release train targets v1.1.6 on macOS 12+ with separate native arm64 and x86_64
artifacts; v1.1.6 removes the fabricated analytics seed data, excludes simulation records from every real analytics surface (portfolio, overview, quant, symbols, equity, pivot, MAE/MFE) and hardens candle persistence under a locked DuckDB; v1.1.5 made the USD position value the only trade sizing (one value field, notional/margin/PnL/R derived from it, approximate labeling for non-USD quotes, the base-quantity entry retired, and the journal reset with a full backup by owner decision); v1.1.4 added the read-only chart review (closed-trade candles and a
...
[`pilot-v1.1.6`](https://github.com/alikula37/kuantra-terminal/releases/tag/pilot-v1.1.6)
...
directs users to `pilot-v1.1.4`; `pilot-v1.1.4` keeps its historical 1.1.4 packages and directs users to `pilot-v1.1.5`; `pilot-v1.1.5` keeps its historical 1.1.5 packages and directs users to `pilot-v1.1.6`. The previous
...
`pilot-v1.1.6` tag is a private prerelease transport identifier, not a canonical product
...
The current matrix is `KTR-001@1.1.6`. The prior `KTR-001@1.0.1` matrix remains as
