...
[`truth-matrix.v1.1.2.json`](truth-matrix.v1.1.2.json). The matrix is a contract, not a
...
The current release train targets v1.1.2 on macOS 12+ with separate native arm64 and x86_64
artifacts; v1.1.2 fixes the in-app update notifier to open this repository's Releases list
instead of a version-pinned page and upgrades the dev-only vitest toolchain to the patched
4.1.11 release. v1.1.1 was the security-hardened rebuild of the v1.1.0 pilot (deep security
...
[`pilot-v1.1.2`](https://github.com/alikula37/kuantra-terminal/releases/tag/pilot-v1.1.2)
...
directs users to `pilot-v1.1.2`. The previous
...
`pilot-v1.1.2` tag is a private prerelease transport identifier, not a canonical product
...
The current matrix is `KTR-001@1.1.2`. The prior `KTR-001@1.0.1` matrix remains as
