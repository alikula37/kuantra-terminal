...
MATRIX = ROOT / "docs" / "release" / "truth-matrix.v1.4.0.json"
...
def test_release_truth_rejects_a_non_matrix_tag():
    result = run_script(CHECKER, "--tag", "v1.4.0-production")
    assert result.returncode != 0
    assert "accepted tag 'v1.4.0'" in result.stderr
...
def test_renderer_excludes_historical_release_claims(tmp_path):
    ...
    assert "Truth & Safety Release" in rendered
